...
import { scene, currentCamera, renderer } from './core.js';
...
// ... (setMode, switchTab, setGlobalHeight, repositionFurniture, deleteFurniture, onWallThicknessChange, onPillarSizeChange, onOpeningPropChange 기존 함수 유지) ...
...
export function onOpeningPropChange(key, val) { if (!state.selectedObject || (state.selectedObject.type !== 'door' && state.selectedObject.type !== 'window')) return; let num = parseFloat(val); if (isNaN(num)) return; state.selectedObject[key] = num; refreshOpeningGeometry(state.selectedObject); if (state.selectedObject.hostWall) refreshWallGeometry(state.selectedObject.hostWall); saveState(); }

// 가구 속성 변경 핸들러
export function onFurniturePropChange(key, val) {
    if (!state.selectedObject || state.selectedObject.type !== 'furniture') return;
    
    let num = parseFloat(val);
    if (isNaN(num) || num <= 0) return; // 유효성 검사
    
    state.selectedObject[key] = num; // width, height, depth 업데이트
    
    // 지오메트리 새로고침 (크기 변경 반영)
    refreshFurnitureGeometry(state.selectedObject);
    saveState();
}
